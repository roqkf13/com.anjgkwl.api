...
from sherlock_homes.adapter.outbound.repositories.juso_db_repository import JusoDbRepository
...
def get_juso_repository(db: AsyncSession = Depends(get_db)) -> JusoRepository:
    return JusoDbRepository(session=db)


def get_juso_use_case(
    repository: JusoRepository = Depends(get_juso_repository),
) -> JusoUseCase:
